...
def generate_report(papers):
    ...
    II. Details Section (STRICT HTML OUTPUT REQUIRED)

For every paper identified in the Index Section, you MUST output the details using EXACTLY the following HTML template. Do NOT use markdown code blocks (```html) or any markdown formatting outside of the HTML tags. Just output the raw HTML.

HTML TEMPLATE:
<div class="paper-item">
<p>[{{INDEX}}] {{PAPER_TITLE_ENGLISH}}</p>
<ul>
  <li><strong>Authors:</strong> {{FULL_NAMES}}</li>
  <li><strong>Link:</strong> <a href="{{URL}}">{{URL}}</a></li>
  <li><strong>Methodology:</strong> <span class="method-tag">{{METHODOLOGY_TYPE}}</span> ({{ADDITIONAL_METHOD_DETAILS}})
    <ul>
    {{OBSERVATION_DETAILS_IF_APPLICABLE}}
    </ul>
  </li>
  <li><strong>Core Results:</strong> {{CORE_RESULTS}}</li>
</ul>
</div>

INSTRUCTIONS FOR PLACEHOLDERS:
1. {{INDEX}}: The paper indices (e.g., 1, 2, 3).
2. {{FULL_NAMES}}: Full names of all authors. 
   - MANDATORY: If an author is a world-renowned expert in the field, you MUST wrap their name exactly like this: <strong style="color:#f95b17;">Name ★(Famous Scholar)</strong>.
3. {{METHODOLOGY_TYPE}}: Choose from [Observation / Simulation / Theory / Methods].
4. {{ADDITIONAL_METHOD_DETAILS}}: Brief context of the method (e.g., Population modeling, dark matter capture).
5. {{OBSERVATION_DETAILS_IF_APPLICABLE}}: 
   - IF the methodology is [Observation], replace this placeholder with two HTML break lines containing the details:
     <li> Energy band: [e.g., X-ray, UV, Optical, Radio, Gamma-ray, Infrared]</li>
     <li> Key Targets/Sample: [source or sample name]</li>
   - IF the methodology is NOT [Observation], replace this placeholder with an empty string (delete it).
6. {{CORE_RESULTS}}: 
   - Provide 2-3 concise, high-impact sentences.
   - MANDATORY: Extract specific physical parameters (e.g., velocity, mass, flux) where available.
   - Preserve all special symbols (like α, β, σ, M☉). You MUST convert all LaTeX-style superscripts and subscripts into HTML tags.
   - DIRECT START: Avoid filler phrases like "This paper presents...". Start directly with the discovery (e.g., "The XRISM/Resolve spectrum reveals a multi-phase outflow with velocity components at...").
